...
export const useRecipeComment = () => {
  ...
  return useMutation<
    any,
    Error,
    { recipeId: string; commentData: FieldValues }
  >({
    mutationKey: ["RECIPE_COMMENT"],
    mutationFn: async ({ recipeId, commentData }) =>
      await commentRecipe(recipeId, commentData),
    onSuccess: (data) => {
...
export const useUpdatedRecipeComment = () => {
  ...
  return useMutation< string,Error, FieldValues>({
    ...
    mutationFn: async ({ commentId, updatedCommentData }) =>
      await updatedRecipeData(commentId, updatedCommentData),
    onSuccess: () => {
